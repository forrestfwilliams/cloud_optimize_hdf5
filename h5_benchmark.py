...
class GetRequestCounter(logging.Handler):
    ...
    def emit(self, record):
        if record.msg.startswith('Sending GET request'):
            self.count += 1
...
def run_benchmarks():
    benchmark(filename, 'Unaltered Local', local=True)
    # benchmark(base_s3_url + filename, 'unaltered s3', local=false)
    # benchmark(base_s3_url + 'nisar_repaged.h5', 'repacked metadata s3', local=false)
